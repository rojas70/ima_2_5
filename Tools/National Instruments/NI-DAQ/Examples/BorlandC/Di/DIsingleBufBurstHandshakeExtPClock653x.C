...
*
* Example program: 
*    DIsingleBufBurstHandshakeExtPClock653x.c
*
* Description: 
...
*    DIG_Grp_Config, NIDAQErrorHandler, DIG_Grp_Mode,
*     Set_DAQ_Device_Info, DIG_Block_In, DIG_Block_Check, NIDAQYield,
...
void main(void)
{
    ...
    iStatus = Set_DAQ_Device_Info(iDevice, ND_CLOCK_REVERSE_MODE_GR1,
     ND_ON);

    iRetVal = NIDAQErrorHandler(iStatus, " Set_DAQ_Device_Info ",
     iIgnoreWarning);

    /* Start the handshaked buffered input of 100 "items". */
    ...
    if (iStatus == 0) {
        ...
        printf(" Digital burst mode handshaked buffered input is done!\n");
