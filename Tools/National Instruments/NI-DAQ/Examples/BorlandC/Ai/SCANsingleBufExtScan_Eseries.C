...
*
* Example program: 
*    SCANsingleBufExtScan_Eseries.c
*
* Description: 
*    Read a waveform from two analog input channels using external scan
*     interval timing (uses low-level NI-DAQ functions, for E-series
*     devices)
*
* Example Category: 
*    AI
*
* Example Task Types: 
*    BUF, NCH, BURST, EXTTIM, ASYNC, INTTRIG, ESER
*
...
*    Select_Signal, NIDAQErrorHandler, DAQ_Config, DAQ_Rate,
*     SCAN_Setup, SCAN_Start, DAQ_Check, NIDAQYield, SCAN_Demux,
...
*     a digital pulse train to the PFI7/STARTSCAN pin.
*
...
void main(void)
{
    ...
    /* Setup for external scan timing conversions into PFI7 with
     iExtScan = 2. */

    iStatus = Select_Signal(iDevice, ND_IN_SCAN_START, ND_PFI_7,
     ND_LOW_TO_HIGH);
    ...
    printf(" Apply your external scan timing pulses to PFI7.\n");
    ...
    iStatus = Select_Signal(iDevice, ND_IN_SCAN_START,
