...
*
* Example program: 
*    DIdoubleBufChangeDetection653x.c
*
* Description: 
*    Inputs digital data continuously into a buffer from a group of two
*     digital ports. Data is latched in when any of the input lines
...
*    iGroup, iGroupSize, iDir, iReqSource, ulPatternMask, iDBModeON,
*     iOldDataStop, ulCount, iHalfReady, ulBufferSize, ulAlignIndex,
*     iResource, iIgnoreWarning
*
...
*     DIG_Block_PG_Config, DIG_Trigger_Config, DIG_DB_Config,
*     Align_DMA_Buffer, DIG_Block_In, DIG_DB_HalfReady,
*     DIG_DB_Transfer, NIDAQYield, DIG_Block_Clear, NIDAQPlotWaveform
*
...
void main(void)
{
    ...
    /* Set up the device for change detection mode The parameters
     iPgTB and iReqInt parameters are ignored in change Detection mode. */
    ...
    /* Set the mask to monitor only the significant lines. 			 	Note :
     if all lines are significant, you do not need to call the 	following
     function. */
    ...
    /* Start the double-buffered change detection input. The
    ...
    if (iStatus == 0) {
        ...
        printf(" Digital double-buffered change detection input is done!\n");
