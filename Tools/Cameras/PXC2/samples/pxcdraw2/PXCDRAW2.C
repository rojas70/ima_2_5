...
static char szTitle[] = "PXC-series Sample 2.  ";
...
BOOL AppIdle(void)
{
	...
	if (GetKeyState(VK_LBUTTON) >= 0)
	{
		...
		if(tagQ[frhIdx])
		{
			...
			tagQ[frhIdx] = 0; // clear the queue handle
		...
		{
			// queue another grab
			tagQ[frhIdx] = pxc.Grab(fgh, frh[frhIdx], (short)(grab_type | QUEUED));
			// flip the previous image and display it
			GetImage(frh[frhIdx]);
			hdc = GetDC(hwndCtl);
...
LONG WINAPI CtlProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_SVIDEO:
			// the PXC200_LC only has s-video on channel 1
