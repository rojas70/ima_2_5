...
/**************************************************************************************/
STDMETHODIMP CControlBasis::get_ref_DesFT_ReferenceValue(short *pVal)
{
	*pVal = (short)m_dFTReferenceValue[m_sFTReferenceValueIndex];

	return S_OK;
}

STDMETHODIMP CControlBasis::put_ref_DesFT_ReferenceValue(short newVal)
{
	// Localvariables
	SAFEARRAY *mysa;
...
STDMETHODIMP CControlBasis::Initialize()
{
	...
	if(m_sWhichIsSubController == SUBORDINATE_FORCE_CONTROLLER)
	{
		...
		m_dFTReferenceValue[0] = Des_Fx;
		m_dFTReferenceValue[2] = Des_Fz;
...
void CControlBasis::SetGains()
{
	...
	{				
		...
		m_dPositionGain[1] = 0.00002;		m_dIKinGain[1] = 0.50;		m_dForceGain[1] = 0.002;		m_dMomentGain[1] =  0; 	
		m_dPositionGain[2] = 0.00002;		m_dIKinGain[2] = 0.50;		m_dForceGain[2] = 0.002;		m_dMomentGain[2] =  0; 	
		...
		m_dPositionGain[5] = 0.00056;		m_dIKinGain[5] = 0.50;		m_dForceGain[5] = 0;			m_dMomentGain[5] =  0.025;// 0.05; // 0.01 //  0.200; 	

		m_dCartesianGain[0] = 0.5;//0.05;
		m_dCartesianGain[1] = 0.5;//0.05;
		m_dCartesianGain[2] = 0.5;//0.05;
		m_dCartesianGain[3] = 0.5;//0.05;
		m_dCartesianGain[4] = 0.5;//0.05;
		m_dCartesianGain[5] = 0.5;//0.05;
	}

	// Gain Tuning for Compounded Controllers
	...
	// Aug 2008
	//
	// In Oct 2008, I discovered an error in the way results were being added for the null space output matrix.
	// The adjustment for the gains is no longer necessary.
/*	if(m_sWhatRobot == HP3JC_ROBOT && m_sWhichIsDomController == DOMINANT_FORCE_CONTROLLER && m_sWhichIsSubController == SUBORDINATE_MOMENT_CONTROLLER)
	{
		m_dMomentGain[3] =   m_dForceGain[0]/2;
		m_dMomentGain[4] =  -m_dForceGain[1]/2;
		m_dMomentGain[5] =   m_dForceGain[2]/2;
	}
*/
	// POSITION/FORCE
...
HRESULT CControlBasis::GetDominantVectorData()
{
	...
	// Check to make sure that all pointers work
...
// CheckError()
//
// This function looks at the error produced by a function. 
// If the error drops below a threshold then it sends out a signal to the state machine
// "Next", so that a transition to the next controller in the set can be activated. 
//***********************************************************************************//
void CControlBasis::CheckError()
{
	// Local variables
	IStateMachine *pIVSStateMachine;
	...
	for(int i=0; i<NUM_AXES; i++)
		CumulativeError += m_dDomCont_Error[i];		
	
	// Print to debut output mode
	ATLTRACE("\nCumulative Error = %d\n", CumulativeError);

	// If the sum of the errors is less than our threshold, then send the SM signal
	if(CumulativeError < 12)
	{
		// (2)  Send signal to the state machine	
		...
		if(m_pIUnkStateMachine != NULL)	
		{
			...
			if(pIVSStateMachine != NULL)
			{
				pIVSStateMachine->Signal(SysAllocString(L"Next"));
				pIVSStateMachine->Release();
			}

			else
				pIVSStateMachine = NULL;			
		}
	}
}

// Display the cumulative error on the interface
STDMETHODIMP CControlBasis::get_CumulativeError(double *pVal)
{
	// TODO: Add your implementation code here
	*pVal = CumulativeError;
	return S_OK;
}

STDMETHODIMP CControlBasis::put_CumulativeError(double newVal)
{
	// TODO: Add your implementation code here
	CumulativeError = newVal;
	return S_OK;
}
...
{
	// Local Variables
	double tempBuff[NUM_AXES]={-10,0,0,0,0,0};
	...
	if(pIVSXYZ != NULL && pIHP3JC != NULL)	
	{
		...
		// Call Incremental move 20 times
		for(int i=0;i<20; i++)
