...
//																  //
// Image is displayed in manager through the use of UpdateManager()/
// Up
////////////////////////////////////////////////////////////////////
#include "stdafx.h"
...
// *NOTE: This may be called more than once after a component is inserted!
	...
	{	
		...
		// These parameters not only affect the size of the image for the frame grabber but also
		// the way the CColorSegemtner class and the CImageProcessing class will read the images
		// For the current state of Sally it seems that 640x480 eats too much of the CPU utilization
		// Go for 320 x 240
		Width	= 320;		// 640;
		Height	= 240;		// 480;
...
HRESULT CPXC_FG::GetImage(LPBYTE Image)
{
	...
	// Another type of if-else statement
	// Expression? value1 : value2
	// If "expression" is true, then value 0, else value 1
	// Hence,
	...
	// If connected to an output image, output data
	// Variables
	long	      lOut  = 0;
	IImageRep*  pImgOut = NULL;

	// Grab pointer to component/object and interface
	   ...
	   if( pImgOut != NULL)	
		{
			// Lock image
			Lock();
			pImgOut->Update(gpBits); //this does a memcpy
			Unlock();
...
HRESULT CPXC_FG::Initial(int FrameGrabberID, int left, int top, int width, int height)
{
	...
	Sleep(2500); /* wait for CCIR autodetect */
	  ...
	  -------------------------------------------------------------------------*/
	  frh[0] = pxc200.AllocateBuffer((short)XResolution,(short)YResolution, PIXEL_TYPE);
	  if(!frh[0]) {
		//Allocate Buffer #0 Failed;
		ATLTRACE("****pxc200.Device**** : Allocate Buffer #0 failed!\n");
		return E_FAIL;
	  }
	  frh[1] = pxc200.AllocateBuffer((short)XResolution, (short)YResolution, PIXEL_TYPE);
	  if(!frh[1]) {
...
void CPXC_FG::GrabImage()
{
	...
	// Compute the length of the image as length x depth = 240*3
	len=XResolution*(((PIXEL_TYPE&0xFF)+7)>>3);
	p = gpBits;

	// Move the through the image, 1 block of 240*3 at at time. 
	for(i=0;i<YResolution;i++)
	{
	  frame.GetRow(frh[frhIdx], p, i);	// Get data from the one of the two CCD panels.
	  p += len;							// Represents moving to the next row, but here we have just one long stream of array elements
	}
...
STDMETHODIMP CPXC_FG::Update(BYTE* pVal)
{ 
	//This function updates the internal image of an IImageRep,
	//However, nothing should be updating the internal image in
	//the frame grabber except an image capture,
	//so this function does nothing.
	ATLTRACE("CPXC_FG::Call to Update of Frame Grabber (doesn't fail, but doesn't do anything)");
...
STDMETHODIMP CPXC_FG::get_FrameGrabberName(BSTR * pVal)
{
	if(FrameGrabberID==0)
		*pVal=SysAllocString(L"Left");
	else if (FrameGrabberID==1)
...
STDMETHODIMP CPXC_FG::put_FrameGrabberName(BSTR newVal)
{
	USES_CONVERSION;
...
/****************************************************************
/* UpdateManager(Interface, Data)
/*
/* This function is called by the Manager IImageRep in ManagerBook
/* It is only called when the representation is seen in ManagerBook
/****************************************************************/
STDMETHODIMP CPXC_FG::UpdateManager(BSTR Interface,VARIANT *Data)
{
	USES_CONVERSION;
	...
	// If an ImageRepresentation is linked here, proceed with this function.
	if(strcmp(OLE2T(Interface),"IImageRep")==0)
	{
		// Set the type for this variant as a variant ARRAY/VARIANT.
		Data->vt=VT_ARRAY|VT_VARIANT;

		// Create a vector of type VT_VARIANT with size of 4. 
		Data->parray=SafeArrayCreateVector(VT_VARIANT,0,4);

		// The 4 elements are: depth, width, height, data
		// Copy the data in the variant over to the arrayptr.
		arrayptr = (VARIANT*)Data->parray->pvData;

		// For the following elements, first set up the type of the data, and then the value.
		arrayptr[0].vt		= VT_I4;
		arrayptr[0].lVal	= Depth;

		arrayptr[1].vt		= VT_I4;
		arrayptr[1].lVal	= Height;

		arrayptr[2].vt		= VT_I4;
		arrayptr[2].lVal	= Width;

		arrayptr[3].vt		= VT_ARRAY|VT_UI1;
		// Create a vector of a size equal to data in image
		arrayptr[3].parray	= SafeArrayCreateVector(VT_UI1,0,Depth*Height*Width);

		// Copy data stored in gpBits over to data structure of the last element of arrayptr.
		memcpy(arrayptr[3].parray->pvData,gpBits,Depth*Height*Width);
	}

	// This interface has not been implmented in ManagerBook yet.
	else if(strcmp(OLE2T(Interface),"IPXC_FG")==0)
	{
...
// We don't use these anymore... but need to keep them cause they are connected with the interfaces of the
// IImageRep interface... this code is very messy and their is lots of unhealthy interdependence...
