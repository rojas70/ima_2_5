...
CNNMuscles01::CNNMuscles01(int m, int leftright)
{
	forwardvalue = m;
	leftorrightarm = leftright;
}

CNNMuscles01::~CNNMuscles01()
{
	fclose(TrainData);
	fclose(weights)   ;
	fclose(hata)     ;
...
void CNNMuscles01::Allocator(void)
{
	using namespace std;
...
void CNNMuscles01::IsExist(void)
{
	if (leftorrightarm) //Left Arm
	{
		if ( forwardvalue ==1 )
		{
			if ((weights=fopen("..\\NNWeights\\LeftNNmuscle01FWeights.txt","w"))==NULL)		
				printf("Can't Open File LeftNNmuscle01FWeights.txt\n");
				
			if( (TrainData=fopen( "..\\TrainFiles\\LeftNeuralDataForward01.txt", "r" )) == NULL )
				printf( "Can't Open File LeftNeuralDataForward.txt\n" );

			if ((hata=fopen("..\\NNWeights\\LeftNNmuscle01FWeightsErrors.txt","w"))==NULL)		
				printf("Can't Open File LeftNNmuscle01FWeightsErrors.txt\n");

		}else 
		{
			if ((weights=fopen("..\\NNWeights\\LeftNNmuscle01BWeights.txt","w"))==NULL)		
				printf("Can't Open File NNmuscle01BWeights.txt\n");
				
			if( (TrainData=fopen( "..\\TrainFiles\\LeftNeuralDataBackward01.txt", "r" )) == NULL )
				printf( "Can't Open File LeftNeuralDataBackward.txt\n" );

			if ((hata=fopen("..\\NNWeights\\LeftNNmuscle01BWeightsErrors.txt","w"))==NULL)		
				printf("Can't Open File LeftNNmuscle01BWeightsErrors.txt\n");
		}
	}
	...
	if (!leftorrightarm) //Right Arm
	{
		if ( forwardvalue ==1 )
		{
			if ((weights=fopen("..\\NNWeights\\RightNNmuscle01FWeights.txt","w"))==NULL)		
				printf("Can't Open File RightNNmuscle01FWeights.txt\n");
				
			if( (TrainData=fopen( "..\\TrainFiles\\RightNeuralDataForward01.txt", "r" )) == NULL )
				printf( "Can't Open File RightNeuralDataForward.txt\n" );

			if ((hata=fopen("..\\NNWeights\\RightNNmuscle01FWeightsErrors.txt","w"))==NULL)		
				printf("Can't Open File RightNNmuscle01FWeightsErrors.txt\n");

		}else 
		{
			if ((weights=fopen("..\\NNWeights\\RightNNmuscle01BWeights.txt","w"))==NULL)		
				printf("Can't Open File RightNNmuscle01BWeights.txt\n");
				
			if( (TrainData=fopen( "..\\TrainFiles\\RightNeuralDataBackward01.txt", "r" )) == NULL )
				printf( "Can't Open File RightNeuralDataBackward.txt\n" );

			if ((hata=fopen("..\\NNWeights\\RightNNmuscle01BWeightsErrors.txt","w"))==NULL)		
				printf("Can't Open File RightNNmuscle01BWeightsErrors.txt\n");
		}
	}
}

void CNNMuscles01::Input(void)
{
	for (t=0;t<(m);t++)
		for (i=0;i<nInput;i++)
			inputx[t][i]  =0;		
	...
	// Here Normalizing the Left Angles. Forward changes between -65 & +25 to 0-1
	if (leftorrightarm)
	for(t=0;t<m;t++)
		inputx[t][0] = (inputx[t][0]+65 ) / 90;

	// Here Normalizing the Right Angles. Forward changes between 55 & -25 to 0-1
	if (!leftorrightarm)
	for(t=0;t<m;t++)
		inputx[t][0] = (inputx[t][0]+25 ) / 80;
}

void CNNMuscles01::Process(void)
{
...
void CNNMuscles01::WeightsWrite(void)
{
	fprintf(weights,"%d\n",nNeuron);
	fprintf(weights,"%f\n",alfa);
	fprintf(weights,"%f\n",mm);
