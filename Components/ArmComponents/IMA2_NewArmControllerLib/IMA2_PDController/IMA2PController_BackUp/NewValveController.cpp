	...
	if(m_bConnected == TRUE)
	{
		...
		if(m_bWhatArm == RIGHT_ARM)
		{
			...
			{			
				...
				// Deflate right front bicep muscle. 
				// Inflate left back bicep muscle. 
...
{	
	...
	if(m_bWhatArm == LEFT_ARM)
	{
		vitalSelectBoard(0);
		for(int m_sChannel=0;m_sChannel<4;m_sChannel++)
			vitalEncoderRead(m_sChannel, &m_dEncoders[m_sChannel]);

		// Switch to Board(1) and first 2 channels
		vitalSelectBoard(1);
		for(m_sChannel=0;m_sChannel<2;m_sChannel++)
			vitalEncoderRead(m_sChannel, &m_dEncoders[m_sChannel+4]);		// Input to index 4 and 5.
...
int CNewValveController::ReadJointAngles()
{
	...
	if(m_bWhatArm == RIGHT_ARM)
	{
		...
		m_dAngles[3] =		  (m_dPhysicalAngle[2] - m_dPhysicalAngle[3] )			  * RADS2DEGREE;	// Roll Angle		
		// We made a mistake in the hardware for the right arm. Angle[4] is Angle[5] and vice-versa.
		// So, the Angle[4] equation belongs to Yaw instead of Pitch. 
		m_dAngles[4] =		  (m_dPhysicalAngle[4] - m_dPhysicalAngle[5] )		      * RADS2DEGREE;	// Yaw angle
		m_dAngles[5] = (0.5 * (m_dPhysicalAngle[4] + m_dPhysicalAngle[5] )	- PI/2 )  * RADS2DEGREE;	// Pitch angle. **Could add a -90 term to have it as old ISAC
	}
...
// 
// This directly affects the Move() function.
/////////////////////////////////////////////////////////////////////////
void CNewValveController::SetHomePositionVoltages()
{
	...
	if(m_bWhatArm == LEFT_ARM)
	{
		...
		m_dInitialValveOutputs[5] = 2.2;	// Front right bicep
		m_dInitialValveOutputs[6] = 2.4;	// Front left bicep
		m_dInitialValveOutputs[7] = 0.0;	// Back left tricep

		m_dInitialValveOutputs[8] = 2.3;	// Back right forearm muscle
		m_dInitialValveOutputs[9] = 1.7;	// Front right forearm muscle
		m_dInitialValveOutputs[10] = 1.7;	// Back left forearm muscle
		m_dInitialValveOutputs[11] = 2.3;	// Front left forearm muscle
	}
	...
	if (m_bWhatArm == RIGHT_ARM)
	{
		...
		m_dInitialValveOutputs[5] = 2.2;	// Left Bicep
		m_dInitialValveOutputs[6] = 2.2;	// Right Bicep
		m_dInitialValveOutputs[7] = 0.0;	// Right Tricep

		m_dInitialValveOutputs[8] = 2.0;	// Left back fore arm muscle
		m_dInitialValveOutputs[9] = 2.0;	// Left front fore arm muscle
		m_dInitialValveOutputs[10] = 2.0;	// Right front fore arm muscle
		m_dInitialValveOutputs[11] = 2.0;	// Right back fore arm muscle
	}
}
/********************************************************************
* ResetVoltages()
...
int CNewValveController::DesiredHomeValues()
{
	...
	{
		MessageBox(NULL, "Binding error", "The binding to DesiredAngles is missing", MB_OK);
		return -1;
	}
		m_pIUnkDesiredAngles->QueryInterface(IID_IVectorSignal,(void**)&pIVSDesAng);
