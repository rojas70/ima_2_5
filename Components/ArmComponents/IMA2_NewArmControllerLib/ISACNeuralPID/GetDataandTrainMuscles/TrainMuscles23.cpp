...
CNNMuscles23::CNNMuscles23(int m, int leftright)
{
	forwardvalue = m;
	leftorrightarm = leftright;
}

CNNMuscles23::~CNNMuscles23()
{
	fclose(TrainData);
	fclose(weights)   ;
	fclose(hata)     ;
...
void CNNMuscles23::Allocator(void)
{
	using namespace std;
...
void CNNMuscles23::IsExist(void)
{
		if (leftorrightarm) //Left Arm
		{
			if ( forwardvalue ==1 )
			{
				if ((weights=fopen("..\\NNWeights\\LeftNNmuscle23FWeights.txt","w"))==NULL)		
					printf("Can't Open File LeftNNmuscle01FWeights.txt\n");
					
				if( (TrainData=fopen( "..\\TrainFiles\\LeftNeuralDataForward23.txt", "r" )) == NULL )
					printf( "Can't Open File LeftNeuralDataForward.txt\n" );

				if ((hata=fopen("..\\NNWeights\\LeftNNmuscle23FWeightsErrors.txt","w"))==NULL)		
					printf("Can't Open File LeftNNmuscle23FWeightsErrors.txt\n");

			}else 
			{
				if ((weights=fopen("..\\NNWeights\\LeftNNmuscle23BWeights.txt","w"))==NULL)		
					printf("Can't Open File NNmuscle23BWeights.txt\n");
					
				if( (TrainData=fopen( "..\\TrainFiles\\LeftNeuralDataBackward23.txt", "r" )) == NULL )
					printf( "Can't Open File LeftNeuralDataBackward23.txt\n" );

				if ((hata=fopen("..\\NNWeights\\LeftNNmuscle23BWeightsErrors.txt","w"))==NULL)		
					printf("Can't Open File LeftNNmuscle23BWeightsErrors.txt\n");
			}
		}
		...
		if (!leftorrightarm) //Right Arm
		{
			if ( forwardvalue ==1 )
			{
				if ((weights=fopen("..\\NNWeights\\RightNNmuscle23FWeights.txt","w"))==NULL)		
					printf("Can't Open File RightNNmuscle23FWeights.txt\n");
					
				if( (TrainData=fopen( "..\\TrainFiles\\RightNeuralDataForward23.txt", "r" )) == NULL )
					printf( "Can't Open File RightNeuralDataForward.txt\n" );

				if ((hata=fopen("..\\NNWeights\\RightNNmuscle23FWeightsErrors.txt","w"))==NULL)		
					printf("Can't Open File RightNNmuscle23FWeightsErrors.txt\n");

			}else 
			{
				if ((weights=fopen("..\\NNWeights\\RightNNmuscle23BWeights.txt","w"))==NULL)		
					printf("Can't Open File RightNNmuscle23BWeights.txt\n");
					
				if( (TrainData=fopen( "..\\TrainFiles\\RightNeuralDataBackward23.txt", "r" )) == NULL )
					printf( "Can't Open File RightNeuralDataBackward23.txt\n" );

				if ((hata=fopen("..\\NNWeights\\RightNNmuscle23BWeightsErrors.txt","w"))==NULL)		
					printf("Can't Open File RightNNmuscle23BWeightsErrors.txt\n");
			}
		}
}

void CNNMuscles23::Input(void)
{
	for (t=0;t<(m);t++)
		for (i=0;i<nInput;i++)
			inputx[t][i]  =0;		
	...
	// Here Normalizing the Angles. Forward changes between 58 - 132 to 0-1 
	for(t=0;t<m;t++)
		inputx[t][0] = (inputx[t][0] - 40) / 85;

}

void CNNMuscles23::Process(void)
{
...
void CNNMuscles23::WeightsWrite(void)
{
	fprintf(weights,"%d\n",nNeuron);
	fprintf(weights,"%f\n",alfa);
	fprintf(weights,"%f\n",mm);
