...
function bodyOnClick()
{
	if (ieVer >= 4) {
		...
		for (; elem; elem = elem.parentElement) {
			...
			if (elem.className == "classNewTipLink")
				return;
		}
...
function initLangs()
{
	...
	if (spans) {
		...
		for (iElem = 0; iElem < spans.length; iElem++) {
			...
			if ((elem.className == "lang") || (elem.id == "newLanguageFilteredSpan")) {
...
function chooseLang(item)
{
	...
	FilterNewLanguageFilteredSections();
	filterLang();
}

function FilterNewLanguageFilteredSections()
{
	var filteredSections = document.all("newLanguageFilteredSection");
	var newLanguageSpans = document.all("newLanguageFilteredSpan");
	
	if((filteredSections!=null) && (filteredSections.length>0))
	{
		for(i=0;i<filteredSections.length;i++)
		{
			if(filteredSections[i].development_language==curLang)
			{
				filteredSections[i].style.visibility="visible";
				filteredSections[i].style.display="block";
			}
			else if(filteredSections[i].development_language!=null)
			{
				filteredSections[i].style.visibility="hidden";
				filteredSections[i].style.display="none";
			}
		}
		if((newLanguageSpans!=null) && (newLanguageSpans.length>0))
		{
		    for(j=0;j<newLanguageSpans.length;j++)
		    {
                newLanguageSpans[j].style.display="none";
		    }
		}
...
function ShowAllNewLanguageFilteredSections()
{
	var filteredSections = document.all("newLanguageFilteredSection");
	var newLanguageSpans = document.all("newLanguageFilteredSpan");
	...
	if((filteredSections!=null) && (filteredSections.length>0))
	{
		...
		if((newLanguageSpans!=null) && (newLanguageSpans.length>0))
		{
		    for(j=0;j<newLanguageSpans.length;j++)
		    {
	            newLanguageSpans[j].style.display="inline";
		    }
		}
	}
}
...
function unfilterLang(name)
{
	...
	ShowAllNewLanguageFilteredSections();
}
