...
if (ToolBar_Supported)
{
	...
	HTMLStr =  "<DIV ID='idToolbar' STYLE='width:100%' >";
	...
	if (!RTL){
		HTMLStr += "<DIV ID='idICPBanner'   STYLE='position:absolute;top:0px;left:0px;height:60px;width:250px;overflow:hidden;vertical-align:top;'><!--BEG_ICP_BANNER--><!--END_ICP_BANNER--></DIV>";
		//HTMLStr += "<DIV ID='idRowSpace1'	STYLE='position:absolute;top:0px;left:250px;height:20px;width:10px;background-color:black;float:right;' NOWRAP><!--Space--></DIV>";
		//This following line inserted by the GSSDEV
		HTMLStr += "<DIV ID='idSpace1'		STYLE='position:absolute;top:0px;left:250px;height:20px;vertical-align:top;overflow:hidden;background-color:#0A6CCE;'><img src='/library/images/support/1ptrans.gif'></DIV>"
		HTMLStr += "<DIV ID='idMSMenuCurve' STYLE='position:absolute;top:0px;left:250px;height:20px;width:18px;overflow:hidden;vertical-align:top;horizontal-align:right;'><IMG SRC='/library/toolbar/images/curve.gif' BORDER=0></DIV>";
		HTMLStr += "<DIV ID='idMSMenuPane'  STYLE='font-size:11px;position:absolute;top:0px;left:250px;height:20px;width:10px;background-color:black;float:right;' ALIGN=RIGHT NOWRAP><!--MS_MENU_TITLES--></DIV>";
		HTMLStr += "</DIV>";
	}else if(RTL){
		...
		HTMLStr += "<DIV ID='idSpace1'		STYLE='position:relative;top:0px;width:100%;left:0;height:20px;vertical-align:top;overflow:hidden;z-index:-1;background-color:#0A6CCE;'><img src='/library/images/support/1ptrans.gif'></DIV>"
		...
		HTMLStr += "<DIV dir='ltr' lang='he' ID='idMSMenuPane'  STYLE='font-size:11px;position:absolute;top:0;left:250;height:20;width:10;color:white;background-color:black;float:left;' NOWRAP><!--MS_MENU_TITLES-->&nbsp;</DIV>";
		HTMLStr += "</DIV>";
	}
	if(!RTL){
		//HTMLStr += "<DIV ID='idRowSpace2' STYLE='position:relative;left:250px;height:40px;'>";
		HTMLStr += "<DIV ID='idRow2' STYLE='position:relative;left:250px;height:40px;'>";
		//This following line inserted by the GSSDEV
		HTMLStr += "<DIV ID='idSpace2'		STYLE='position:absolute;top:0px;left:0px;height:40px;vertical-align:top;overflow:hidden;background-color:#0A6CCE;'><img src='/library/images/support/1ptrans.gif'></DIV>"
		...
		HTMLStr += "<DIV ID='idMSCBanner'   STYLE='position:absolute;top:0px;left:180px;height:40px;width:200px;vertical-align:top;overflow:hidden;' ALIGN=RIGHT><!--BEG_MSC_BANNER--><!--END_MSC_BANNER--></DIV>";
		HTMLStr += "</DIV>";
	}else if(RTL){
		HTMLStr += "<DIV ID='idRow2' STYLE='position:relative;left:0;height:40px;'>" ; // RTL Correction: was left:250
		HTMLStr += "<DIV ID='idSpace2'		STYLE='position:absolute;top:0px;left:0px;height:40px;vertical-align:top;overflow:hidden;background-color:#0A6CCE;'><img src='/library/images/support/1ptrans.gif'></DIV>"
		...
		HTMLStr += "<DIV ID='idMSCBanner'   STYLE='position:absolute;top:0;left:200;height:40;width:200;vertical-align:top;overflow:hidden;' ALIGN=LEFT><!--BEG_MSC_BANNER--><!--END_MSC_BANNER--></DIV>";
		HTMLStr += "</DIV>";
	}	
	if(!RTL){
		// for GSS purpose we disabled the idRow3. No ICP Menu bar from this file.
		HTMLStr += "<DIV ID='idRow3' STYLE='position:relative;height:20px;width:100%;display:none'>";
		// zIndex Correction
	...
	ToolbarBGColor	= "#0A6CCE";						// toolbar background color
	
	if (Strict_Compat)
	{
		MSFont  = "10px Arial";
		ICPFont = "bold 10px Verdana";
	}
	else
	{
		if( !RTL ){
			...
			MSFont  = "12px Arial";	 // RTL Correction: (was Verdana)
			ICPFont = "bold 12px Arial"; // RTL Correction: (was Verdana)
			//MNP1 RTL revision
			MSFTFont = "12px Arial";
			ICPFTFont = "bold 12px Arial"
			//end
		}	
	}
...
function resizeToolbar()
{
	...
	if ( !RTL )
	{
		...
		{	
			...
			idSpace2.style.width =	Math.max(realWidth, MSMenuWidth) - MSMenuWidth + 74 + 'px';
			idSpace2.style.backgroundColor = ToolbarBGColor;
			idSpace1.style.backgroundColor = ToolbarBGColor;
						
			idMSMenuPane.style.width  = MSMenuWidth  + 'px';
			idADSBanner.style.left    = (w+18)  + 'px';
			idMSCBanner.style.left    = (w+18)  + 'px';
			idMSCBanner.style.width   = (MSMenuWidth - 126)  + 'px';
			idICPMenuPane.style.width = ToolbarMinWidth + w  + 'px';
		}
	}
	else if( RTL )
	{
		idToolbar.style.left = document.body.clientWidth - realWidth;
		idToolbar.style.width= realWidth;
		...
		idMSCBanner.style.width   = Math.max(MSMenuWidth, 200) - 200;
		...
		idSpace1.style.backgroundColor = ToolbarBGColor;
		// start -  added by gsa
		// idSpace1.style.width = (w + 18) + 'px';
		// idSpace2.style.width = (w + 18 + 200) + 'px';
		// end-  added by gsa
	}
}
...
{	tempGif = "/library/images/support/" + Gif;
	...
	if (ToolbarMenu != null && ToolbarMenu != StartMenu) 
	{
		...
		if (!RTL){
			...
			{// RTL Correction: cY-8 instead of cY
				if ( cY-8 >= (y - idRow2.offsetHeight) && cY < y) // Another RTL Correction: idRow2 instead of idRow3 since hidden
...
{
	//document.writeln("<" + "script language='JavaScript' src='/libary/toolbar/loc/global-" + toolbarLocale + ".js' type='text/javascript'></" + "script>");
