...
Begin VB.UserControl EditTab 
   ...
   ScaleHeight     =   5910
   ScaleWidth      =   9300
   ToolboxBitmap   =   "EditTab.ctx":0000
...
Attribute VB_Exposed = True
    ...
    'Added 8-31-05 So that clients know when a tab has changed. Raise the OnSwitch
    'event *before* changing tbs.Tabs.SelectedItem (so that the client knows
    'what the old tab was (the old selected item) as well as the new tab
    '(the argument for the event) and can adapt accordingly.
    Public Event OnSwitch(NewTab As MSComctlLib.Tab)
    ...
    'Added 8-31-05 -CLC Because Roberto never did it.
    Private Sub mnuTab_Delete_Click()
        Dim T As MSComctlLib.Tab
        Dim Cancel As Boolean
        Set T = tbs.SelectedItem
        Cancel = False
        If (tbs.Tabs.Count) = 1 Then
            If StrComp(T.Caption, "Page 1") = 0 Then
                Call tbs.Tabs.Add(2, "Page 2", "Page 2")
            Else
                Call tbs.Tabs.Add(2, "Page 1", "Page 1")
            End If
            RaiseEvent OnNew(tbs.Tabs(2), Cancel)
            RaiseEvent OnSwitch(tbs.Tabs(2))
            Set tbs.SelectedItem = tbs.Tabs(2)
        ElseIf T.Index = 1 Then
            RaiseEvent OnSwitch(tbs.Tabs(2))
            Set tbs.SelectedItem = tbs.Tabs(2)
        Else
            RaiseEvent OnSwitch(tbs.Tabs(T.Index - 1))
            Set tbs.SelectedItem = tbs.Tabs(T.Index - 1)
        End If
        RaiseEvent OnDelete(T, Cancel)
        tbs.Tabs.Remove T.Index
    End Sub

    Private Sub mnuTab_New_Click()
        Dim T       As MSComctlLib.Tab
        Dim Cancel  As Boolean
        Set T = tbs.Tabs.Add(tbs.SelectedItem.Index + 1, "New Tab", "New Tab")
        Cancel = False
        RaiseEvent OnNew(T, Cancel)
        RaiseEvent OnSwitch(T)
        Set tbs.SelectedItem = T
    ...
    Private Sub tbs_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
        ...
        For Each T In tbs.Tabs
            ...
            If InX And InY Then
                If StrComp(tbs.SelectedItem.Caption, T.Caption) <> 0 And (Button And vbLeftButton) Then
                    RaiseEvent OnSwitch(T) 'Added this to let clients know the tab has switched -CLC
                    Set tbs.SelectedItem = T
                End If
                Exit Sub
            End If
