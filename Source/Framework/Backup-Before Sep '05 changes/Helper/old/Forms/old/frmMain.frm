...
Begin VB.Form frmMain 
   ...
   Begin MSComctlLib.StatusBar stb 
      ...
      EndProperty
   End
   Begin IMA2_DAD.Toolbar Toolbar1 
      Align           =   1  'Align Top
      Height          =   375
      Left            =   0
      TabIndex        =   1
      ...
      _ExtentY        =   661
   End
   Begin IMA2_DAD.AgentTreeview atv 
      Height          =   5910
      Left            =   0
      TabIndex        =   4
      Top             =   1680
      ...
      BorderStyle     =   0
   End
   Begin IMA2_DAD.AgentListview alv 
      Height          =   5925
      Left            =   2595
      TabIndex        =   2
      Top             =   1680
    ...
    Private Sub NewComponent()
        ...
        Else
            ...
            For I = LBound(CDs) To UBound(CDs)
                Call CreateIMA2Component(P + "\" + CDs(I).Path, CDs(I).PID, , True, atv.Locator)
                If Err Then
...
'*********************************************************************************************************************************************************************
    ...
    Private Sub atv_NodeMenu(ByVal Node As MSComctlLib.Node)
       If Node Is Nothing Then Exit Sub
        Call CheckATVMenus(Node.Key, Node.Image = II_Container)
        PopupMenu mnuCmp
    End Sub
...
'*********************************************************************************************************************************************************************
    ...
    Public Function ReadProperty(O As Object, PN As String) As Variant
        ...
        If Err Or (VarType(V) And vbArray) Then
            Err.Clear
            V = "(...)"
...
'*********************************************************************************************************************************************************************
    ...
    Private Sub Form_Resize()
        ...
        ElseIf Not atv.Visible Then
            alv.Move BP_Left, BP_Top, BP_Width, BP_Height
        Else
            atv.Move BP_Left, BP_Top, atv.Width, BP_Height
            slr.Move atv.Left + atv.Width, BP_Top, slr.Width, BP_Height
            alv.Move slr.Left + slr.Width, BP_Top, Me.ScaleWidth - slr.Width - atv.Width - (Bevel1.Width * 1), BP_Height
        End If
...
'*********************************************************************************************************************************************************************
    ...
    Private Sub mnuCmp_SaveAs_Click()
        ...
        With Dlg
            ...
            .FilePath = "Untitled"
            .Paths = GetSavedPaths()
            'set dlg.tvw.ImageList = atv.
            For I = 1 To Dlg.Nodes.Count Step 1
                .Nodes(I).Image = atv.Nodes(.Nodes(I).Key).Image
            Next I
...
'*********************************************************************************************************************************************************************
    ...
    Private Sub CheckALVMenus(Path As String, ItemSelected As Boolean, IsContainer As Boolean)
        ...
        mnuCmp_Cut.Visible = PL > PL_NETWORK
        mnuCmp_Copy.Visible = mnuCmp_Cut.Visible
        mnuCmp_Paste.Visible = mnuCmp_Cut.Visible
        
        mnuCmp_Cut.Enabled = ItemSelected And (PL > PL_LOCATOR)
        mnuCmp_Copy.Enabled = mnuCmp_Cut.Enabled
        ...
        mnuCmp_Paste_Sep.Visible = mnuCmp_Cut.Visible And (mnuCmp_Delete.Visible Or mnuCmp_SaveAs.Visible Or mnuCmp_Properties.Visible)
    ...
    Private Sub CheckATVMenus(Path As String, IsContainer As Boolean)
        ...
        mnuCmp_Cut.Enabled = (PL > PL_LOCATOR)
        mnuCmp_Copy.Enabled = mnuCmp_Cut.Enabled
        mnuCmp_Paste.Enabled = (PL > PL_NETWORK)
