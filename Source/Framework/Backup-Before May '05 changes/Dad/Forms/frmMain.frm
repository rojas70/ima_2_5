...
Object = "{81605258-1A3B-47EE-A9BE-4F1963575A0F}#2.0#0"; "IMA2_DadX.ocx"
...
'*********************************************************************************************************************************************************************
    ...
    Private Sub NewComponent()
        ...
        Else
            ...
            ReDim CDs(frmInsert.DescriptionLBound To frmInsert.DescriptionUBound)
            For I = LBound(CDs) To UBound(CDs)
                Set CDs(I) = frmInsert.DescriptionIndex(I)
            Next I
            'Original code here was CDs=frmInsert.Descriptions
            ...
            For I = LBound(CDs) To UBound(CDs)
                ...
                If frmInsert.Context = ICC_Local Then
                    Dim PLA As ILocatorAdvanced                                     'Parent Locator (advanced)
                    Set PLA = g_AL.Locate(LocatorFromPath(P + "\" + CDs(I).Path))   'Get the parent container's locator
                    Set C = CreateObject(CDs(I).PID)                                'Create the local component
                    Call C.Construct(P + "\" + CDs(I).Path, CDs(I).PID, PLA)        'Construct the component
                ...
                End If
                'Original code: Set C = CreateIMA2Component(P + "\" + CDs(I).Path, CDs(I).PID, , True, atv.Locator)
...
'                If MsgBox("Deleting an activated component from the IMA2 network will release its bindings and shut down the component, possibly causing instability in other agents. Are you sure you wish to unregister " + Path + " while it is activated?", vbOKCancel, "Delete Activated Component") <> vbOK Then Exit Sub
 '               C.Activated = False
  '              If MsgBox("Are you sure you wish to delete " + Path + " from the IMA2 network?", vbOKCancel Or vbInformation, "Delete Deactivated Component") <> vbOK Then Exit Sub
            ...
            If C.Distributing.IsContainer Then
                ...
                If (x = vbOK) Then
                    ...
                    'This code might not work when the tree veiw is updating itself quickly
                    'because it gets the next node by referencing the component just deleted.
                        DeleteComponent childnode.Key
...
'*********************************************************************************************************************************************************************
    ...
    Private Sub SetBindings(Item As ListItem)
        ...
        Set C = g_AL.Locate(atv.Path)                                       'Locate the component
        If C Is Nothing Then  'Added Mar '05 to remove errors due to tree-view refresh problems. -CLC
            atv.Refresh
            atv.ExpandAll
            Exit Sub
        End If
        Set frmBindings.NetList = m_Net                                     'Set up the bindings form
        frmBindings.Refresh
