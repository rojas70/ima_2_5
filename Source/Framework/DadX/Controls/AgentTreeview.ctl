...
Attribute m_Net.VB_VarHelpID = -1
    ...
    Dim m_CollapsedNodes    As New Collection
...
'************************************************************************************************
    ...
    Private Sub m_Net_OnRefresh()
        ...
        Dim FirstVis As String
        Dim ColPath As Variant
        Dim N As Node
        Dim LocPaths As New Collection
        ...
        If (tvw.Nodes.Count > 0) Then   'Checks for first creation of tvw
            I = 1
            While (tvw.Nodes(I).Visible = False)
                I = I + 1
            Wend
            FirstVis = tvw.Nodes(I).Key
        Else
            FirstVis = "\\"        'On first creation, set first visible to root
        End If
        ...
        For Each ColPath In m_CollapsedNodes
            Call LocPaths.Add(ColPath, ColPath)
        Next ColPath
        ...
        tvw.Nodes(FirstVis).EnsureVisible
        If Err Then
            tvw.Nodes("\\").EnsureVisible
            Err.Clear
        End If
        
        For Each ColPath In LocPaths
            Set N = tvw.Nodes.Item(ColPath)
            If N Is Nothing Then
                Call m_CollapsedNodes.Remove(ColPath)
            Else
                tvw.Nodes(ColPath).Expanded = False
            End If
        Next ColPath
...
'************************************************************************************************
    ...
    Public Property Get Path() As String
        ...
        Path = tvw.SelectedItem.Key                                     'Returns path to component
    End Property
...
'************************************************************************************************
    ...
    Private Sub RemoveNode(Path As String)
        ...
        Call m_CollapsedNodes.Remove(Path)
        RaiseEvent NodeClick(tvw.SelectedItem)
    End Sub
...
'************************************************************************************************
    ...
    Private Sub tvw_AfterLabelEdit(Cancel As Integer, NewString As String)
        m_Editing = False
        Dim Key As String
        Key = ParentFromPath(tvw.SelectedItem.Key) & "\" & NewString
        ...
        Else
            Refresh
            Path = Key
            RaiseEvent NodeClick(tvw.Nodes(Key))
        End If
    End Sub
...
'        Set tvw.SelectedItem = tvw.Nodes(NN.Key)
'        m_ButtonDown = True
        'Disabled dragging in atv 2/1/06. It was messing with the list refresh. -CLC
'        m_DragPath = tvw.HitTest(X, Y).Key
'        tmrDrag.Enabled = True
'
'        If (Button And vbLeftButton) Then RaiseEvent NodeClick(NN)
...
'    End Sub
    ...
    Private Sub tvw_Collapse(ByVal Node As Node)
        Call m_CollapsedNodes.Add(Node.Key, Node.Key)
    End Sub

    Private Sub tvw_Expand(ByVal Node As Node)
        On Error Resume Next
        Call m_CollapsedNodes.Remove(Node.Key)
    End Sub
