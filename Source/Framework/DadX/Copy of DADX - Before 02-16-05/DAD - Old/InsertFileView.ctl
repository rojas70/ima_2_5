...
Begin VB.UserControl InsertFileView 
   ...
   Begin IMA2_DAD.SimplePanel sp 
      ...
      Begin VB.Image img 
         ...
         Index           =   3
         Left            =   120
         Picture         =   "InsertFileView.ctx":0000
         Top             =   1440
         Width           =   240
      End
      Begin VB.Image img 
         ...
         Index           =   2
         Left            =   120
         Picture         =   "InsertFileView.ctx":014A
         Top             =   140
         Width           =   240
      End
      Begin VB.Image img 
         ...
         Index           =   1
         Left            =   120
         Picture         =   "InsertFileView.ctx":0294
         Top             =   945
         Width           =   240
      End
      Begin VB.Image img 
         ...
         Index           =   0
         Left            =   120
         Picture         =   "InsertFileView.ctx":03DE
         Top             =   630
         Width           =   240
      End
   ...
   Begin VB.CommandButton cmdFile 
      DisabledPicture =   "InsertFileView.ctx":0528
      DownPicture     =   "InsertFileView.ctx":086A
      Height          =   285
      Left            =   5160
      Picture         =   "InsertFileView.ctx":09B4
      Style           =   1  'Graphical
      TabIndex        =   0
      Top             =   30
      Width           =   300
...
'************************************************************************************************
    ...
    Private Sub tvw_AfterLabelEdit(Cancel As Integer, NewString As String)
        ...
        OldPath = tvw.SelectedItem.FullPath
        CDs = m_CF.Descriptions
        Set CD = CDs(tvw.SelectedItem.Index - 1)
        
        If StrCount(tvw.SelectedItem.FullPath, "\") = 0 Then
            NewPath = NewString
        Else
            NewPath = ParentFromPath(OldPath) + "\" + NewString
        End If
        
        Dim I As Integer
        For I = LBound(CDs) To UBound(CDs)
            CDs(I).Path = Replace(CDs(I).Path, OldPath + "\", NewPath + "\")
            If CDs(I).Path = OldPath Then CDs(I).Path = NewPath
        Next I
