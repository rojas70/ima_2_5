...
'************************************************************************************************
    ...
    Private Sub UserControl_Resize()
        ...
        lvw.ColumnHeaders(3).Width = UserControl.ScaleWidth - lvw.ColumnHeaders(1).Width - lvw.ColumnHeaders(2).Width - 3
    End Sub
    ...
    Private Sub AddItem(Path As String)
        ...
        Else
            ...
            ElseIf TypeOf C Is IContainer Then
                Set LI = lvw.ListItems.Add(, Path, NameFromPath(Path), , II_Container)
                LI.SubItems(1) = C.PID
                LI.ForeColor = QBColor(0)
            Else
                Set LI = lvw.ListItems.Add(, Path, NameFromPath(Path), , m_Icons.IconIndex(C.PID))
                LI.SubItems(1) = C.PID
                LI.ForeColor = QBColor(0)
            End If
        End If
